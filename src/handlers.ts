...
export async function fetchHandler(req: Request, app: any) {
  ...
  if (/basic|c?webp|multiple/gi.test(modifiers) && !/f_webp/.test(modifiers)) {
    if (req.method === 'POST') {
  ...
  return ipxOptimizeHandler(req, modifiers, dataURI, app);
}

export async function imageminWebpPostHandler(req: Request) {
  try {
    let files;
    ...
    if (!files || (files && files.length === 0)) {
      return Response.json(
        { error: { message: 'No files provided', httpStatus: 400 } },
        { status: 400 },
      );
    }
    ...
    const results = await Promise.all(
      files
        ...
        .map(async (file: File | Blob) => {
          ...
          return {
            hash,
            output: String(`[${new Uint8Array(optimized)}]`),
            type: 'image/webp',
            name: file.name,
          };
        }),
    ...
    return Response.json(
      { error: { message: `Failure: ${err.message}`, httpStatus: 500 } },
      { status: 500 },
    );
  }
}

export async function imageminWebpGetHandler(req: Request, dataURI: string) {
  ...
  const url = new URL(req.url);
  const options = Object.fromEntries([...url.searchParams.entries()].map(([k, v]) => [k, v]));
  try {
...
export async function ipxOptimizeHandler(req: Request, modifiers: string, data: string, app: any) {
  const url = new URL(req.url);
  // const [modifiers, data] = url.pathname.slice(1).split('/');
  ...
  if (data.includes('data:')) {
    ...
    const resp = await toWebHandler(app)(clonedReq);
